...
def main():
    ...
    while True:
        ...
        sidebar_canvas = renderer.draw_sidebar(maze_canvas.shape[0], current_algo, stats, CURRENT_SEED, player_game_over)
        ...
        if algo_trigger:
            ...
            if search_func:
                ...
                for i in range(0, len(visited_order), step):
                    ...
                    temp_sidebar = renderer.draw_sidebar(temp_maze.shape[0], current_algo, stats, CURRENT_SEED, player_game_over)
        ...
        if not player_game_over:
            ...
            if player_pos == maze.end_pos:
                ...
                stats["player_time"] = time.time() - player_start_time
                print(f"   # 通關！最終手動時間：{stats['player_time']:.2f} 秒。")
