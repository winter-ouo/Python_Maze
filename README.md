# Python_Maze

![OpenCV](https://img.shields.io/badge/OpenCV-4.13-green?style=flat-square)
![Python](https://img.shields.io/badge/Python-3.10-yellow?style=flat-square)
![Numpy](https://img.shields.io/badge/Numpy-2.2-blue?style=flat-square)
![Pygame](https://img.shields.io/badge/Pygame-2.6-orange?style=flat-square)

## 此為Demo分支中的Demo版(具備迷霧驅散功能)


本專案是一個基於 Python 與 OpenCV 開發的互動式迷宮遊戲。核心系統採用隨機 Prim 演算法生成具備高度挑戰性的破碎地形，並結合多種經典的演算法進行動態可視化展示。此外，遊戲還提供整合了戰爭迷霧、隱蔽鑰匙機制與巡邏怪物的雙人合作生存模式。
#### 簡報連結：[https://canva.link/dosfk4twqmknz6w](https://canva.link/dosfk4twqmknz6w)
---
## 專案目錄結構
```
.
├── .gitignore
├── README.md
├── venv/
├── assets/                  # 遊戲影音資源資料夾
│   ├── back.mp3             # 背景音樂 (BGM)
│   ├── click.mp3            # 點擊按鈕聲
│   ├── Died.mp3             # 瑪利歐死亡聲
│   ├── key.mp3              # 撿到鑰匙 (金幣) 聲
│   ├── move.mp3             # 走路腳步聲
│   ├── search.mp3           # AI 尋路聲
│   └── victory.mp3          # 通關勝利聲
└── src/
    ├── __pycache__/
    ├── fog_effect.py        # 迷霧效果，透過 OpenCV 與 NumPy 矩陣遮罩運算，動態渲染以玩家為中心的圓形可視範圍
    ├── key.py               # 管理遊戲中的鑰匙機制，在關卡初始化時利用尋路演算法將鑰匙配置於必經之路外的區域，並處理玩家拾取判定
    ├── main.py              # 主程式，管理遊戲主選單、遊戲畫面之間的狀態切換、處理滑鼠與鍵盤事件監聽
    ├── maze_generator.py    # 隨機迷宮生成(隨機 Prim 演算法)
    ├── maze_model.py        # 定義迷宮的基礎資料結構，儲存地圖矩陣、起點與終點座標，並執行玩家移動時的碰撞牆壁判定
    ├── monster.py           # 控制生存模式中的巡邏怪物，包含怪物的動態生成時間、沿特定垂直通道來回巡邏的移動邏輯
    ├── pathfinding.py       # 演算法模組，實作 A* 搜尋、廣度優先搜尋 (BFS)、深度優先搜尋 (DFS) 以及左手沿牆法 (Wall Follower)
    └── renderer.py          # 畫布繪製，將迷宮方格、數據面板、演算法足跡、閃爍道具與角色圓球整合輸出至 OpenCV 視窗
```
---

## 核心功能說明

### 1. 基礎迷宮生成與演算法可視化 (Default Mode)
* **隨機 Prim 演算法：** 生成具有大量分支、短路徑且無規律方向的迷宮地形，避免產生規律性的直線性通道。
* **演算法可視化：** 支援 A* 搜尋、廣度優先搜尋 (BFS)、深度優先搜尋 (DFS) 以及左手沿牆法 (Wall Follower)。系統會記錄並即時渲染演算法的探索軌跡（黃色足跡）與最終路徑（綠色線條），並於右側面板統計執行時間與探索節點數。

### 2. 迷霧模式 (Fog Mode)
* **迷霧遮擋：** 使用矩陣遮罩運算，依據玩家當前座標動態生成圓形可視區域，未探索區域保持全黑。支援單人與雙人獨立視野疊加。
* **鑰匙機制：** 系統會在初始時透過 A* 計算起點到終點的必經之路，並強制將鑰匙放置於非必經之路的岔路或死胡同中。鑰匙具備隱蔽特性，每 6 秒會穿透迷霧現形 2 秒。
* **怪物巡邏：** 玩家移動 5 秒後觸發怪物生成。怪物會自動鎖定地圖中的連續垂直通道，並在範圍內進行上下反彈巡邏。

---

## 操作說明

### 系統環境要求
* 操作系統：Windows
* 開發語言：Python 3.10
* 核心依賴庫：OpenCV-Python, NumPy, pygame

### 鍵盤控制指南

| 按鍵 / 模式 | 預設模式 (Default Mode) | 迷霧模式 (Fog Mode) |
| :--- | :--- | :--- |
| **W, A, S, D** | 控制玩家一（藍色球體）移動 | 控制玩家一（藍色球體）移動 |
| **方向鍵** | 控制玩家一（藍色球體）移動 | 控制玩家二（綠色球體）移動（僅 2P 模式） |
| **數字鍵 1** | 觸發 A* 搜尋演算法展示 | 無功能 |
| **數字鍵 2** | 觸發 BFS 尋路演算法展示 | 無功能 |
| **數字鍵 3** | 觸發 DFS 尋路演算法展示 | 無功能 |
| **數字鍵 4** | 觸發 Wall Follower 演算法展示 | 無功能 |
| **R 鍵** | （遊戲結束後）以全新種子重置地圖 | （遊戲結束後）以全新種子重置地圖 |
| **ESC 鍵** | 返回遊戲主選單 | 返回遊戲主選單 |
